...
public class updateUserAvatar extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet updateUserAvatar</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet updateUserAvatar at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (acc != null) {
            String avatar = (String) uploadImage.uploadFile(request, "img");
            if (!avatar.isEmpty()) {
                acc.setUser_image(avatar);
                updateProfileDAO.updateProfileAvatar(avatar, email);
                request.setAttribute("mess", "Update Successfully!");
                request.getRequestDispatcher("profileuser").forward(request, response);
            } else {
                request.setAttribute("mess", "File Upload Error!");
                request.getRequestDispatcher("profileuser").forward(request, response);
            }
        }
    }
