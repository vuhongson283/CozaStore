...
public class wishlist extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (acc == null) {
            request.setAttribute("mess", "Please login to add wish list!");

            request.getRequestDispatcher("login?mod=1").forward(request, response);

        } else {
            String uid = acc.getUser_id();

            if (action.equalsIgnoreCase("add")) {
                Wishlist check = dao.checkWishList(uid, pid);
                if (check == null) {
                    dao.addToWishList(uid, pid);
                } else {
                    request.setAttribute("messwish", "You have added the product to wish list.");
                }
                request.getRequestDispatcher("ShowProductDetail?pid=" + pid).forward(request, response);

            } else if (action.equalsIgnoreCase("remove")) {
                dao.removeItemWishList(uid, pid);
